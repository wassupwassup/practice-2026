...
2. [Заключение]
3. https://disk.yandex.ru/d/tVMTCm1CO8l-5g видео демонстрация игры
---
...
## 1.1. Создание игрового окна и цикла
Начнём с основ, создание окна в котором будет происходить действие нашей игры и прописание игрового цикла.
```python
import pygame
import sys
...
```
## 1.2 Система частиц
Добавим систему частиц, чтобы наша игра выглядела красочней и красивее.
```python
...
```
## 1.3 Загрузка спрайтов 
У каждого объекта в игре должна быть своя модель или же спрайт, это и перснаж, NPC, враги, подбираемые предметы и т.д.
```python
...
## 1.4 Класс игрока
Давайте создадим нашего персонажа и зададим ему параметры.
```python
class Player:
...
```
## 1.5 Класс Enemy
Создадим врагов, которых нужно будет победить, также запишем их параметры.
```python
class Enemy:
...
## 1.6 Класс NPC
В качестве дополнения можно добавить пару-тройку неигровых персонажей для разнообразия геймплея.
```python
class NPC:
...
## 1.7 Класс Предмета (Item)
Создадим несколько видов подбираемых предметов.
```python
class Item:
...
Система взаимодействия игрока с NPC.
```python
class DialogueBox:
...
```
## 1.9 Боевая система
А вот и суть игры, если есть противники, значит должна быть система, которая позволяет с ними бороться.
```python
class CombatSystem:
...
Создадим условие победы и поражения.
```python
class VictoryScreen:
    def __init__(self, screen):
        self.screen = screen
